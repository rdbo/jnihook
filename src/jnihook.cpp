...
JNIHook_Init(JavaVM *jvm)
{
		...
		bool is_oracle_jvm = false;
        if (!jvm_flag_type_result) {
				jvm_flag_type_result = VMType::from_static("Flag");
				if(!jvm_flag_type_result) {
        ...
        for (size_t i = 0; i < numFlags; ++i) {
                auto flag = VMType::from_instance(is_oracle_jvm ? "Flag" : "JVMFlag", &flags_buf[i * jvm_flag_size]);
